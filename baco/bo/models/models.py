...
from baco.param.transformations import transform_data, preprocess_parameters_array, transform_estimate
    ...
    for i, metric in enumerate(param_space.metric_names):
        ...
        if settings["models"]["model"] == "gaussian_process":
            if settings["GP_model"] == "gpy":
                from baco.bo.models.gpgpy import GpGpy
                model = GpGpy(settings, X, y)
            elif settings["GP_model"] == "botorch":
                from baco.bo.models.gpbotorch import GpBotorch
                model = GpBotorch(settings, X, y)
            elif settings["GP_model"] == "gpytorch":
                from baco.bo.models.gpgpytorch import GpGpytorch
